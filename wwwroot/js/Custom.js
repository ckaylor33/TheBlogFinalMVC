...
function AddTag() {
    ...
    if (searchResult != null) {
        //Trigger sweetalert for whatever condition is contained in the searchResult var
...
//The search function will detect either an empty or a duplicate Tag on the same post
    ...
    if (tagsEl) {
        ...
        for (let index = 0; index < options.length; index++) {
            if (options[index].value == str) {
                return `The Tag #${str} was detected as a duplicate and not permitted on this post!`
            }
        }
    }
...
const swalWithDarkButton = Swal.mixin({
    customClass: {
        confirmButton: 'btn btn-danger btn-small btn-outline-dark',
    },
